...
> [!NOTE]
> **Hướng dẫn Chuyển đổi từ Firebase sang PostgreSQL:**
> - Không sử dụng cấu trúc Firestore Documents lồng nhau hay dynamic references.
> - Toàn bộ quan hệ dữ liệu đã được chuẩn hoá sang khóa ngoại quan hệ (`UUID`) trong PostgreSQL.
> - Các trường liên quan đến Số lượng VĐV đăng ký thực tế được tính trực tiếp từ Database thông qua trường `_count.participants` (thay vì read count từ Firebase array).
> - Phân trang (Pagination) sử dụng chuẩn `limit`, `offset` và trả về `meta` chứa thông tin tổng bản ghi.

### 3.1 Lấy danh sách giải đấu công khai (Public Tournaments List)
...
- **Query Parameters:**
  ...
  - `categoryId` (optional): `string` (lọc theo môn thể thao/thể loại)
  - `status` (optional): `string` (`REGISTRATION_OPEN`, `UPCOMING`, `IN_PROGRESS`, `COMPLETED`)
- **Response Body (200 OK):**
  ```json
  {
    "data": [
      {
        ...
        "parentId": "parent-uuid-if-exists",
        "name": "Giải đấu Mùa Hè 2026 - Đơn Nam",
        ...
        "venue": {
          ...
          "locationAddress": "Sư Vạn Hạnh, Quận 10, TP.HCM"
        },
        "_count": {
          "participants": 8
        },
        "divisions": [
          {
            "id": "sibling-uuid",
            "name": "Giải đấu Mùa Hè 2026 - Đôi Nam Nữ",
            "matchType": "DOUBLE",
...
### 3.2 Lấy chi tiết một giải đấu (Get Tournament Detail)
...
- **Response Body (200 OK):**
  ```json
  {
    ...
    "name": "Giải đấu Mùa Hè 2026 - Đơn Nam",
    "description": "Giải đấu quy mô bán chuyên",
    "prizeDescription": "Tổng giải thưởng lên đến 10.000.000 VNĐ",
    "contactInfo": "Liên hệ BTC qua SĐT 0900112233",
    "bannerUrl": "https://res.cloudinary.com/...",
    "logoUrl": "https://res.cloudinary.com/...",
    "status": "REGISTRATION_OPEN",
    "tournamentType": "PUBLIC",
