...
class ApiTeamRepository implements ITeamRepository {
  ...
  Future<List<FootballTeamSummary>> listMyFootballTeams() async {
    ...
    return raw
        .whereType<Map>()
        .map(
          (row) => FootballTeamSummary.fromJson(Map<String, dynamic>.from(row)),
        )
        .toList();
  }

  Future<FootballTeamSummary> createFootballTeam({
    required String name,
    required String categoryId,
  }) async {
    final response = await _dioClient.dio.post(
      '/football-teams',
      data: {'name': name.trim(), 'categoryId': categoryId},
    );
    final raw = response.data is Map ? response.data['data'] : response.data;
    if (raw is! Map) {
  ...
  Future<FootballTeamSummary> updateFootballTeam(
    String teamId, {
    String? name,
    String? logoUrl,
    String? status,
  ...
  Future<FootballTeamSummary> uploadFootballTeamLogo(
    String teamId,
    List<int> bytes,
    String fileName,
  ) async {
    ...
    final uploadData = upload.data is Map
  ...
  Future<List<Map<String, dynamic>>> searchFootballTeamMembers(
    String teamId,
    String query,
  ) async {
  ...
  Future<void> updateFootballTeamMember(
    String teamId,
    String userId,
    String role,
  ) async {
    ...
    yield* Stream.periodic(const Duration(seconds: 15))
        .asyncMap((_) async {
          try {
            ...
            _log.error(
              'Keeping cached teams after polling failure',
              error,
              stack,
            );
          }
  ...
  Future<List<Team>> getAllByTournament(String tournamentId) async {
    ...
    try {
      ...
      dynamic divResponse;
      try {
        divResponse = await _dioClient.dio.get(
      ...
      if (response.statusCode == 200) {
        ...
        return list.map((json) {
          ...
          final List<String> members = rosters
              .map(
                (r) =>
                    (r['fullName'] ?? r['user']?['fullName'] ?? '').toString(),
              )
          ...
          final memberInfos = rosters
              ...
              .map(
                (roster) =>
                    MatchMemberInfo.fromJson(Map<String, dynamic>.from(roster)),
              )
          ...
          final divId =
              json['tournamentDivisionId']?.toString() ??
...
class FootballTeamSummary {
  ...
  List<FootballTeamMemberSummary> get activeMembers => members
      .where(
        (member) =>
            member.status == null || member.status!.toUpperCase() == 'ACTIVE',
      )
      .toList();
  ...
  factory FootballTeamSummary.fromJson(Map<String, dynamic> json) {
    ...
    final membership = json['membership'] is Map
        ? Map<String, dynamic>.from(json['membership'])
        : null;
    return FootballTeamSummary(
      ...
      name:
          ...
          lookupAppLocalizations(
            PlatformDispatcher.instance.locale,
          ).clubRankingTeamFallback,
      categoryId:
          team['categoryId']?.toString() ??
          team['category_id']?.toString() ??
          '',
      ...
      members: (team['members'] as List<dynamic>? ?? const [])
          .whereType<Map>()
          .map(
            (item) => FootballTeamMemberSummary.fromJson(
              Map<String, dynamic>.from(item),
            ),
          )
          .toList(),
      eloPoints:
          ((json['rank'] is Map ? json['rank']['eloPoints'] : null) as num?)
              ?.toInt() ??
          1000,
      peakElo:
          ((json['rank'] is Map ? json['rank']['peakElo'] : null) as num?)
              ?.toInt() ??
          1000,
      matchesPlayed:
          ((json['rank'] is Map ? json['rank']['matchesPlayed'] : null) as num?)
              ?.toInt() ??
          0,
      matchesWon:
          ((json['rank'] is Map ? json['rank']['matchesWon'] : null) as num?)
              ?.toInt() ??
          0,
      winStreak:
          ((json['rank'] is Map ? json['rank']['winStreak'] : null) as num?)
              ?.toInt() ??
          0,
    );
  }
...
class FootballTeamMemberSummary {
  const FootballTeamMemberSummary({
  ...
  factory FootballTeamMemberSummary.fromJson(Map<String, dynamic> json) =>
      FootballTeamMemberSummary(
