...
class _AdminVerificationScreenState
  ...
  Widget _buildStatusFilter(AppColorsExtension colors, AppLocalizations? l10n) {
    final statuses = [
      ...
      (
        'pending',
        l10n!.adminVerificationStatusPending,
        const Color(0xFFF59E0B),
      ),
      (
        'verified',
        l10n!.adminVerificationStatusVerified,
        const Color(0xFF10B981),
      ),
      (
        'rejected',
        l10n!.adminVerificationStatusRejected,
    ...
    return requestsAsync.when(
      data: (requests) {
        final filtered = requests.where((r) {
          if (_typeFilter != 'all' &&
              r['type']?.toString().toLowerCase() != _typeFilter)
            return false;
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, i) =>
              _buildVerificationCard(context, filtered[i], colors),
        );
      },
      ...
      error: (e, _) => Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              l10n!.adminVerificationLoadError,
              style: TextStyle(
                fontSize: 16,
                fontWeight: FontWeight.w700,
                color: colors.textPrimary,
  ...
  Widget _buildVerificationCard(
    ...
    final name =
        request['name']?.toString() ?? request['fullName']?.toString() ?? '---';
  ...
  Future<void> _handleReject(String? id, AppColorsExtension colors) async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(
            ...
            child: Text(l10n!.adminVerificationCancel),
          ),
          ElevatedButton(
    ...
    try {
      ...
      await dio.patch(
        ...
        data: {'status': 'REJECTED', 'rejectedReason': reasonCtrl.text.trim()},
      );
      ref.invalidate(_adminVerificationProvider);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
...
final _adminVerificationProvider = FutureProvider<List<Map<String, dynamic>>>((
  ref,
) async {
  final dio = ref.read(dioProvider);
