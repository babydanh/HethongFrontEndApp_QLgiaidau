...
class _AdminChangeRequestsScreenState
    extends ConsumerState<AdminChangeRequestsScreen> {
  ...
  Widget _buildFilterChips(AppColorsExtension colors, AppLocalizations? l10n) {
    final filters = [
      ...
      (
        'pending',
        l10n!.adminChangeRequestsFilterPending,
        const Color(0xFFF59E0B),
      ),
      (
        'approved',
        l10n!.adminChangeRequestsFilterApproved,
        const Color(0xFF10B981),
      ),
      (
        'rejected',
        l10n!.adminChangeRequestsFilterRejected,
      ...
      data: (requests) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, i) =>
              _buildRequestCard(context, filtered[i], colors),
        );
      },
      ...
      error: (e, _) => Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              l10n!.adminChangeRequestsLoadError,
              style: TextStyle(
                fontSize: 16,
                fontWeight: FontWeight.w700,
                color: colors.textPrimary,
  ...
  Widget _buildRequestCard(
    ...
    final type =
        request['type']?.toString() ?? (l10n!.adminChangeRequestsTypeOther);
    final description = request['description']?.toString() ?? '';
    final requestedBy =
        request['requestedBy']?['fullName']?.toString() ??
        (l10n!.adminChangeRequestsRequester);
          ...
          if (status == 'PENDING') ...[
            ...
            Row(
              children: [
                Expanded(
                  child: _actionBtn(
                    l10n!.adminChangeRequestsApprove,
                    Icons.check_rounded,
                    const Color(0xFF10B981),
                    () => _handleAction(request['id'], 'APPROVED', colors),
                  ),
                ),
                const SizedBox(width: 8),
                Expanded(
                  child: _actionBtn(
                    l10n!.adminChangeRequestsReject,
                    Icons.close_rounded,
                    const Color(0xFFEF4444),
                    () => _handleAction(request['id'], 'REJECTED', colors),
                    outlined: true,
                  ),
                ),
              ],
...
final _adminChangeRequestsProvider = FutureProvider<List<Map<String, dynamic>>>(
  (ref) async {
    final dio = ref.read(dioProvider);
